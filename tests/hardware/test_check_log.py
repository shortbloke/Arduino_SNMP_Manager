...
def good_log():
    ...
    for cycle in range(50):
        for stage in range(4):
            lines += [
                f"MEM cycle={cycle} stage={stage} free=20000 largest=19000 "
...
class LogChecks(unittest.TestCase):
    ...
    def test_incomplete_or_failed(self):
        ...
        for log in [
            ...
            valid.replace(
                "OID version=2 .1.3.6.1.2.1.1.1.0", "OID version=2 .1.3.6.1.2.1.1.2.0", 1
            ),
            "\n".join(
                line for line in valid.splitlines() if not line.startswith("MEM cycle=1 stage=1 ")
            ),
        ]:
