...
def main():
    ...
    parser.add_argument(
        "--small", action="store_true", help="Only short back-to-back bursts with no loop delay"
    )
    args = parser.parse_args()
    with (
        serial.Serial(args.serial, 115200, timeout=0.2) as board,
        ...
        def run(name, count, gap, pause, payload, inform):
            ...
            result = dict(
                ...
                packet_bytes=len(packet(1, payload, inform)),
                inform=inform,
                send_seconds=round(send_seconds, 3),
                acknowledged=len(acknowledgements),
                bad_ack=bad_ack,
                empty_datagrams=empty_datagrams,
                initial_unique=initial_unique,
                initial_acknowledged=initial_acknowledged,
        ...
        loads = ((5, 0), (10, 0), (20, 0)) if args.small else ((20, 0.01), (100, 0.001), (500, 0))
        for pause in pauses:
            for payload in payloads:
                for inform in (False, True):
                    for count, gap in loads:
                        run("burst", count, gap, pause, payload, inform)
                        run("recovery", 10, 0.1, 0, 32, True)
        record({"done": True})
