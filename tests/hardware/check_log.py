...
def check(text):
    ...
    for line in text.splitlines():
        ...
        memory = re.fullmatch(
            r"MEM cycle=(\d+) stage=(\d+) free=(\d+) largest=(\d+) "
            r"minSampledFree=(\d+) minSampledBlock=(\d+)",
            line,
        )
        if memory:
        ...
        ending = re.fullmatch(
            r"DONE cycles=(\d+) failures=(\d+); inspect MEM trend and OID sets", line
        )
        if ending:
    ...
    for number in range(done):
        ...
        if any(a >= b for a, b in zip(numeric, numeric[1:], strict=False)):
            raise ValueError("Non-increasing OIDs")
        if any(not any(c == number and s == stage for c, s, _, _ in samples) for stage in range(4)):
            raise ValueError("Missing per-stage memory measurements")
    return {
        "cycles": done,
        "stages": len(results),
        "minimum_sampled_free_heap": min(s[2] for s in samples),
        ...
        "last_cycle_end_heap": next(
            s[2] for s in reversed(samples) if s[0] == done - 1 and s[1] == 3
        ),
    }
