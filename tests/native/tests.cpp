...
int main(int argc, char **argv)
{
    ...
    auto add = [&](const char *name, std::function<void()> f, bool regression = false)
    { tests.push_back({name, regression, f}); };
    add("wifi_example handles fresh samples timeouts and counter boundaries",
        []
        {
            checkExampleHelpers<wifi_example::Counter32Rate, wifi_example::PollState<2>>();
            CHECK(wifi_example::nextRequestID(32767) == 1);
            CHECK(wifi_example::nextRequestID(0) == 1);
        });
    add("ethernet_example handles fresh samples timeouts and counter boundaries",
        []
        {
            ...
            CHECK(ethernet_example::nextRequestID(0) == 1);
        });
    add("multi_example handles fresh samples timeouts and counter boundaries",
        []
        {
            checkExampleHelpers<multi_example::Counter32Rate, multi_example::PollState<2>>();
            CHECK(multi_example::nextRequestID(32767) == 1);
            CHECK(multi_example::nextRequestID(0) == 1);
        });
    add("manager and requests share registration lifetime",
        []
        {
            ...
            Request request;
            {
                ...
                manager.tryAddHandler(callback);
                request.tryAddOIDPointer(callback);
                CHECK(request.build());
                CHECK(request.build());
            }
            CHECK(destroyed == 0);
    ...
    add("bounded primitive decoding and serialization reject short buffers",
        []
        {
            ...
            sequence.tryAddValueToList(new IntegerType(42));
    ...
    add("legacy custom BER children serialize without null-buffer measurement",
        []
        {
            struct Legacy : BER_CONTAINER
            {
                Legacy() : BER_CONTAINER(true, INTEGER) {}
            ...
            ComplexType tree(STRUCTURE);
            tree.addValueToList(new Legacy());
            unsigned char bytes[5] = {};
            CHECK(tree.serialise(bytes) == 5);
            CHECK(Bytes(bytes, bytes + 5) == Bytes({0x30, 3, 2, 1, 42}));
            memset(bytes, 0xcc, sizeof(bytes));
            CHECK(tree.serialise(bytes, sizeof(bytes)) < 0);
            for (auto byte : bytes)
                CHECK(byte == 0xcc);
        });
    add("signed legacy request IDs correlate matching replies",
        []
        {
            const std::vector<std::pair<short, Bytes>> identifiers = {
                {-32768, {0x80, 0}}, {-1, {0xff}}, {0, {0}}, {32767, {0x7f, 0xff}}};
            for (const auto &id : identifiers)
            {
                ...
                request.setRequestID(id.first);
                CHECK(request.sendTo(udp.peer));
                udp.incoming = tlv(0x30, join({{2, 1, 1},
                                               tlv(4, {'p', 'u', 'b', 'l', 'i', 'c'}),
                                               tlv(0xa2, join({tlv(2, id.second),
                                                               {2, 1, 0},
                                                               {2, 1, 0},
                                                               tlv(0x30, binding({2, 1, 42}))}))}));
                manager.loop();
                CHECK(value == 42);
            }
        });
    add("caller-created float callbacks retain the legacy storage convention",
        []
        {
            float value = 0;
            IntegerCallback callback;
            callback.isFloat = true;
            callback.value = reinterpret_cast<int *>(&value);
            callback.OID = const_cast<char *>(oid);
            UDP udp;
            callback.ip = udp.peer;
            Manager manager;
            manager.setUDP(&udp);
            manager.addHandler(&callback);
            udp.incoming = message(binding({2, 1, 7}));
            manager.loop();
            CHECK(value > 0.69f && value < 0.71f);
        });
    add("legacy addHandler borrows caller-owned callbacks and OID text",
        []
        {
            int value = 0;
            IntegerCallback callback;
            ...
            CHECK(callback.value == &value && std::string(callback.OID) == oid);
        });
    add("native-width numeric callback overloads write through the actual destination type",
        []
        {
            Manager integerManager;
            UDP integerUDP;
            integerManager.setUDP(&integerUDP);
            long integer = 0;
            CHECK(integerManager.addIntegerHandler(integerUDP.peer, oid, &integer));
            integerUDP.incoming = message(binding({2, 1, 0xff}));
            integerManager.loop();
            CHECK(integer == -1);
            for (int tag : {0x41, 0x42, 0x43})
            {
                Manager manager;
                UDP udp;
                manager.setUDP(&udp);
                ...
                if (tag == 0x41)
                    CHECK(manager.addCounter32Handler(udp.peer, oid, &value));
                if (tag == 0x42)
                    CHECK(manager.addGaugeHandler(udp.peer, oid, &value));
                if (tag == 0x43)
                    CHECK(manager.addTimestampHandler(udp.peer, oid, &value));
                udp.incoming = message(binding(tlv(tag, {0, 0xff, 0xff, 0xff, 0xff})));
                manager.loop();
                CHECK(value == UINT32_MAX);
            }
            Manager wideManager;
            UDP wideUDP;
            wideManager.setUDP(&wideUDP);
            unsigned long long wide = 0;
            CHECK(wideManager.addCounter64Handler(wideUDP.peer, oid, &wide));
            wideUDP.incoming =
                message(binding(tlv(0x46, {0, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff})));
            wideManager.loop();
            CHECK(wide == UINT64_MAX);
        });
    add("legacy outgoing capacity remains larger than the receive buffer",
        []
        {
            Manager manager;
            UDP udp;
            ...
            for (int i = 0; i < 50; ++i)
                request.addOIDPointer(callback);
            CHECK(request.sendTo(udp.peer));
            CHECK(udp.outgoing.size() > SNMP_PACKET_LENGTH &&
                  udp.outgoing.size() < SNMP_PACKET_LENGTH * 3);
        });
    add("deep copy allocation failure preserves the source and previous destination",
        []
        {
            ComplexType source(STRUCTURE);
            source.addValueToList(new IntegerType(42));
            for (int allowance = 0; allowance < 8; ++allowance)
            {
                ComplexType destination(STRUCTURE);
                destination.addValueToList(new IntegerType(7));
                {
                    FailAllocations fail(allowance);
                    destination = source;
                }
                CHECK(static_cast<IntegerType *>(source._values->value)->_value == 42);
                auto copied = static_cast<IntegerType *>(destination._values->value)->_value;
                CHECK(copied == 7 || copied == 42);
            }
        });
    add("legacy polling recovers after more lost replies than tracking slots",
        []
        {
            Manager manager;
            UDP udp;
            ...
            request.setUDP(&udp);
            request.addOIDPointer(callback);
            for (int i = 1; i <= SNMP_MAX_PENDING_REQUESTS * 3; ++i)
            {
                request.setRequestID(i);
                CHECK(request.sendTo(udp.peer));
            }
            udp.incoming =
                message(binding({2, 1, 42}), 1, "public", 0xa2, SNMP_MAX_PENDING_REQUESTS * 3);
            manager.loop();
            CHECK(value == 42);
            udp.incoming = message(binding({2, 1, 9}), 1, "public", 0xa2, 1);
            manager.loop();
            CHECK(value == 42); // Evicted late replies cannot update the value.
        });
    add("copying owners preserves independent lists and BER trees",
        []
        {
            Manager original;
            UDP udp;
            original.setUDP(&udp);
            int value = 0;
            auto *callback = original.addIntegerHandler(udp.peer, oid, &value);
            {
                Manager copy(original), assigned;
                assigned = original;
                CHECK(copy.callbacks != original.callbacks && copy.callbacks->value == callback);
                Request request;
                request.addOIDPointer(callback);
                CHECK(request.build());
                SNMPGet duplicate(request);
                CHECK(duplicate.packet != request.packet);
                request.clearOIDList();
                CHECK(duplicate.callbacks->value == callback);
                CHECK(duplicate.build());
                SNMPGet assignedRequest("public", 0);
                assignedRequest = duplicate;
                CHECK(assignedRequest.callbacks != duplicate.callbacks);
                IntegerCallback callbackCopy(*static_cast<IntegerCallback *>(callback));
                CHECK(callbackCopy.OID != callback->OID && std::string(callbackCopy.OID) == oid);
            }
            udp.incoming = message(binding({2, 1, 42}));
            original.loop();
            CHECK(value == 42);
            ComplexType tree(STRUCTURE);
            tree.addValueToList(new IntegerType(7));
            const ComplexType source(tree);
            ComplexType copy(source);
            CHECK(copy._values != source._values);
            static_cast<IntegerType *>(copy._values->value)->_value = 9;
            CHECK(static_cast<IntegerType *>(source._values->value)->_value == 7);
            auto wire = message(binding({2, 1, 42}));
            SNMPGetResponse response;
            CHECK(response.parseFrom(wire.data(), wire.size()));
            SNMPGetResponse duplicate(response), assigned;
            assigned = response;
            CHECK(duplicate.SNMPPacket != response.SNMPPacket &&
                  duplicate.varBinds != response.varBinds);
            CHECK(assigned.communityString != response.communityString);
            CHECK(response.parseFrom(message({}).data()));
            CHECK(static_cast<IntegerType *>(duplicate.varBinds->value->value)->_value == 42);
            CHECK(static_cast<IntegerType *>(assigned.varBinds->value->value)->_value == 42);
        });
    add("UDP receive never transmits during cleanup and resumes after rejected packets",
        []
        {
            Manager manager;
            UDP udp;
            ...
            CHECK(udp.transmissions == 0 && udp.flushes == 0);
        });
    ...
    add("owning packet objects remain copyable and moving releases the old packet",
        []
        {
    ...
    add("legacy short request ID and port remain available",
        []
        {
            Manager manager;
            UDP udp;
            ...
            Request request;
            request.setUDP(&udp);
            request.tryAddOIDPointer(manager.addIntegerHandler(udp.peer, oid, &value));
            request.setRequestID(32767);
            request.setPort(static_cast<short>(65535));
            CHECK(request.sendTo(udp.peer));
            CHECK(udp.destinationPort == 65535);
            SNMPGetResponse decoded;
            CHECK(decoded.parseFrom(udp.outgoing.data(), udp.outgoing.size()));
            CHECK(decoded.requestID == 32767u);
    ...
    add("allocation failures leave requests and parsers reusable",
        []
        {
            ...
            Request request;
            request.setUDP(&udp);
            CHECK(request.tryAddOIDPointer(callback));
            auto wire = message(binding({2, 1, 42}));
    ...
    add("request golden wire and ports",
        []
        {
            for (int version : {0, 1})
            {
                ...
                Request r(version);
                r.setUDP(&udp);
                r.tryAddOIDPointer(m.addIntegerHandler(udp.peer, oid, &value));
    ...
    add("request list ordering and clearing",
        []
        {
            ...
            Request r;
            r.tryAddOIDPointer(a);
            r.tryAddOIDPointer(b);
    ...
    add("successful Get preserves OID order on wire",
        []
        {
            ...
            Request r;
            r.setUDP(&u);
            r.tryAddOIDPointer(m.addIntegerHandler(u.peer, oid, &n));
            r.tryAddOIDPointer(m.addIntegerHandler(u.peer, ".1.3.6.1.2.1.1.3.0", &n));
    ...
    add("bounded response parser rejects every truncated prefix",
        []
        {
            ...
            manager.loop();
            CHECK(value == 99 && udp.incoming.empty() && udp.flushes == 0 &&
                  udp.transmissions == 0);
        });
    ...
    add("request tracking is independent per callback and requires a complete send",
        []
        {
            ...
            one.setUDP(&udp);
            two.setUDP(&udp);
            one.tryAddOIDPointer(a);
            two.tryAddOIDPointer(b);
    ...
    add("sequence content exactly 256 has a two-octet length",
        []
        {
            ...
            for (int i = 0; i < 128; ++i)
            {
                sequence.tryAddValueToList(new NullType());
    ...
    add("sequence lengths either side of 256",
        []
        {
            for (size_t contentLength : {127, 128, 255, 257})
            {
                ...
                ComplexType sequence(STRUCTURE);
                sequence.tryAddValueToList(value);
    ...
    add("nested BER ownership destroys each child once",
        []
        {
            ...
            int destroyed = 0;
            {
                ...
                nested->tryAddValueToList(new TrackedInteger(destroyed));
                root.tryAddValueToList(nested);
                root.tryAddValueToList(new TrackedInteger(destroyed));
            }
            CHECK(destroyed == 2);
        });
    ...
    for (const auto &test : tests)
    {
        ...
        testing::RegisterTest(test.regression ? "Regression" : "Baseline", name.c_str(), nullptr,
    ...
    for (auto &t : tests)
        if (t.regression == regressions)
        {
            ...
            int status = 0;
            bool ok = child > 0 && waitpid(child, &status, 0) == child && WIFEXITED(status) &&
                      WEXITSTATUS(status) == 0;
            if (!ok)
                ++failed;
            std::cout << (ok ? "PASS " : "FAIL ") << t.name;
            if (child > 0 && WIFSIGNALED(status))
