...
with tempfile.TemporaryDirectory(prefix="snmp-headers-") as directory:
    ...
    for macro, value in [
        ("SNMP_PACKET_LENGTH", "768"),
        ("SNMP_OCTETSTRING_MAX_LENGTH", "512"),
        ("MAX_OID_LENGTH", "192"),
        ("SNMP_VALUE_MAX_LENGTH", "512"),
        ("SNMP_MAX_PENDING_REQUESTS", "2"),
        ("DEBUG", "1"),
        ("DEBUG_BER", "1"),
        ("SUPPRESS_ERROR_FAILED_PARSE", "1"),
    ]:
        define = "-D" + macro + "=" + value
        run(define, "-c", str(client), "-o", str(work / "client.o"))
        result = subprocess.run(
            compiler
            + [str(work / "client.o"), str(work / "default.o"), "-o", str(work / "mismatch")],
            capture_output=True,
            text=True,
        )
        if result.returncode == 0 or "BuildConfiguration" not in result.stderr:
            raise RuntimeError(
                "Configuration mismatch was not rejected: " + macro + "\n" + result.stderr
            )
    ...
    run(
        "-I" + str(work),
        '-DSNMP_CONFIG_HEADER="settings.h"',
        str(client),
        config,
        "-o",
        str(work / "shared-header"),
    )
