...
public:
    ...
    int beginPacket(IPAddress ip, uint16_t p) { ++packets; destination=ip; destinationPort=p; outgoing.clear(); return 1; }
    size_t write(const unsigned char* p, size_t n) { outgoing.insert(outgoing.end(),p,p+n); return n; }
