...
def parts(version):
    ...
    return base + ((3, 0) if not suffix else
                   ({"alpha": 0, "beta": 1, "rc": 2}[suffix.split('.')[0]], int(suffix.split('.')[1])))
...
def check_branch(branch, version):
    ...
    if branch not in {"master", *expected}:
        raise ValueError("Release only from main, master, release/1.x, release/2.x or feature/friendly-query-api")
    if parts(version)[0] not in (1, 2) or (branch in expected and parts(version)[0] != expected[branch]):
...
def prepare(root, version, branch):
    ...
    properties = (root / "library.properties").read_text()
    (root / "library.properties").write_text(re.sub(r"^version=.*$", "version=" + version, properties, flags=re.M))
    # Update current-version guidance, without rewriting historical changelog entries.
    readme = (root / "README.md").read_text()
    (root / "README.md").write_text(re.sub(r"(?<![0-9.])" + re.escape(old) + r"(?![0-9.])", version, readme))
